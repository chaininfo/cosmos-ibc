...
func (c relayCommander) refine(bz []byte, sequence int64, passphrase string) []byte {
	...
	ctx := context.NewCoreContextFromViper().WithDecoder(authcmd.GetAccountDecoder(c.cdc)).WithSequence(sequence)
	ctx = ctx.WithNodeURI(viper.GetString(FlagToChainNode))
	//ctx := context.NewCoreContextFromViper().WithDecoder(authcmd.GetAccountDecoder(c.cdc))
	ctx, err := ctx.Ensure(ctx.FromAddressName)
